	...
	- #### High order functions: filter, map y reduce
		- **Filter**
			...
			- [¿cómo usar python filter?](https://www.digitalocean.com/community/tutorials/how-to-use-the-python-filter-function-es)
				...
				- ```python
				  def filterByKey(key, value, data):
				      def filterData(x):
				          for k, v in x.items():
				              if (k == key.lower()) & (v == value.lower()):
				                  return True
				      return filter(filterData, data)
				  
				  def run():
				      print(list(filterByKey("language", "python", DATA)))
				  ```
		- **Map**
		...
		- **Reduce**
			...
			- ==Lambda lleva dos parámetros==, serían el primer elemento de la lista y el segundo.
			- ```python
